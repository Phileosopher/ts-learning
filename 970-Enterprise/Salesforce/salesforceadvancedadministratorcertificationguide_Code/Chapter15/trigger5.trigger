     ...
     if(Trigger.isBefore){
         ...
         if(Trigger.isUpdate || Trigger.isInsert){
            ...
            if(currentOpp.AccountId != null) {
                ...
                Account acc = [Select Id From Account Where Id = :currentOpp.AccountId];
                //gets the sum of the amounts of all opportunities 
                //that will be probably colosing this month
                Double totalAmount = 0;
                List<Opportunity> oppList = [Select Id, Amount From Opportunity 
                                        Where AccountId = :currentOpp.AccountId
                                        and CloseDate = THIS_MONTH
                                        and StageName != 'Closed Lost'];
                //iterates trhough the opportunity list
                for(Opportunity opp : oppList){
                    totalAmount += opp.Amount;
                }

                //sets the value on the Account object and update it
                acc.This_Month_All_Deals__c = totalAmount;
                update acc;
            }
